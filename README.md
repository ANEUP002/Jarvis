...
Open apps, open websites, type text, click at coordinates, press keyboard shortcuts, scroll, take screenshots. Basically full desktop automation built in.

**Runs multiple agents in parallel for complex tasks**
Simple question → one agent answers directly. Complex task → Chief Agent breaks it into subtasks, runs Research + Code + Writer agents in parallel, then combines the results. You can watch the whole pipeline in the HUD.
...
The Classifier first tries keyword rules (fast, no LLM call). If it's confident, it skips the LLM entirely. If not, it falls back to an LLM classifier. Simple questions that start with "what", "who", "how" are always kept simple — no unnecessary multi-agent overhead.
...
- Research answers → saved under `memory/notes/research/`
- Code outputs → saved under `memory/notes/code/`
- Task results → saved under `memory/notes/tasks/`
- Emails sent → logged under `memory/notes/emails/`

Each note has a title, tags, category, creation date, and `[[wiki-style links]]` to related notes. The system maintains a backlinks index so you can see which notes reference which.

When you ask a new question, before going to the LLM it does a vector similarity search over all past notes. If it finds something relevant, that context gets injected into the prompt. Over time, JARVIS gets smarter about topics you've asked about before.

The vector store is built with sentence-transformers running locally — no API key needed for embeddings.

---

## Self-Learning

The learning engine (`app/learning.py`) runs silently on every message. It does a few things:

1. **Fact extraction** — regex patterns pull out your name, employer, location, ongoing projects, people you mention
2. **Interest tracking** — counts topic keywords over time to build a profile of what you care about
3. **Preference learning** — if you ask for shorter answers or more detail, it remembers
4. **Session memory** — conversation history is saved to disk and loaded on startup, so JARVIS has context from last time
5. **Explicit memory commands** — you can say "remember that I'm applying for jobs", "forget that", "what do you know about me"
...
You need Python 3.10+ and Windows (voice/audio uses Windows-specific APIs).
...
The only key you actually need is `OPENROUTER_API_KEY`. Get one free at [openrouter.ai](https://openrouter.ai) — it gives you access to GPT-4o, Gemini, Claude, and a bunch of free models all in one.

Everything else (Groq, MiniMax, Google AI, Together) is optional. The system falls back through providers automatically if one fails.
...
| "Open Chrome" / "Open VS Code" | Launches the app |
...
| "What time is it" / "What's today's date" | Instant local answer |
...
JARVIS has full desktop automation. You can use these directly in the terminal:
...
type hello world        → types text into focused window
...
The desktop panel shows a live architecture diagram that updates as tasks run. You can see exactly which component is active:

- **INPUT → CLASSIFY → CHIEF** — always lit up when a task is running
- **RESEARCH / CODE / WRITER** — whichever agent is handling your request glows
- **Multi-agent mode** — all three can light up simultaneously for parallel tasks
- Model name shown under the active agent
- Task type and complexity shown under the classifier node
...
JARVIS uses OpenRouter as the primary gateway, which gives access to many models through one API key. The model selection is automatic based on task type and complexity:
...
If a model fails (429, 404, timeout), it automatically falls back to the next one in the list without the user noticing.

---

## What's not included in the repo

- `memory/` — your personal profile and notes (private, auto-created on first run)
- `logs/` — runtime logs
- `queue/` — task queue files
- `vector_db/` — generated embeddings
- `assets/voices/` — Piper and Whisper model files (large, download separately)
- `.env` — your API keys
...
**Faster Whisper** (better speech recognition, runs locally):
```bash
pip install faster-whisper
# model downloads automatically on first use (~40MB for tiny.en)
```

**Piper TTS** (local neural voice, much better than Windows TTS):
```bash
pip install piper-tts
# Download a voice model from https://rhasspy.github.io/piper-samples/
# Put .onnx and .onnx.json files in assets/voices/piper/
```

**OpenAI TTS** (highest quality, requires API key):
```bash
# Add to .env:
OPENAI_API_KEY=sk-...
